...
Item {
    ...
    Grid {
        ...
        MetricCard {
            ...
            rows: [
                { label: qsTr("Temperatura"), value: root.temp(root.cpu?.temp_c) },
                { label: qsTr("Frecuencia"), value: root.cpu?.freq_mhz ? `${root.number(root.cpu.freq_mhz, 0)} MHz` : "—" },
                { label: qsTr("Gobernador"), value: root.cpu?.governor ?? "—" }
            ]
        }
        ...
        MetricCard {
            ...
            rows: [
                { label: qsTr("Usada"), value: root.gb(root.memory?.used_gb) },
                { label: qsTr("Disponible"), value: root.gb(Math.max(0, Number(root.memory?.total_gb ?? 0) - Number(root.memory?.used_gb ?? 0))) },
        ...
        MetricCard {
            ...
            rows: [
                { label: qsTr("Usado"), value: root.gb(root.disk?.used_gb) },
                { label: qsTr("Libre"), value: root.gb(root.disk?.free_gb) },
                { label: qsTr("Dispositivo"), value: root.snapshot?.disk_io?.device ?? "/" }
            ]
        }
        ...
        MetricCard {
            ...
            subtitle: root.gpuAt(0)?.name ?? qsTr("Sin datos de GPU")
            progress: root.gpus.length > 0 ? Number(root.gpuAt(0)?.usage ?? 0) / 100 : -1
            rows: [
                { label: qsTr("Temperatura"), value: root.temp(root.gpuAt(0)?.temp_c) },
                { label: qsTr("VRAM"), value: `${root.gb(root.gpuAt(0)?.vram_used_gb)} / ${root.gb(root.gpuAt(0)?.vram_total_gb)}` },
                { label: qsTr("Potencia"), value: root.watt(root.gpuAt(0)?.power_w) }
            ]
        }
        ...
        MetricCard {
            ...
            subtitle: root.gpus.length > 1
                ? (root.gpuAt(1)?.name ?? qsTr("Segunda GPU"))
                : `${qsTr("Descarga")} ${root.speed(root.network?.rx_mbps)}`
            progress: root.gpus.length > 1 ? Number(root.gpuAt(1)?.usage ?? 0) / 100 : -1
            rows: root.gpus.length > 1
                ? [
                    { label: qsTr("Temperatura"), value: root.temp(root.gpuAt(1)?.temp_c) },
                    { label: qsTr("VRAM"), value: `${root.gb(root.gpuAt(1)?.vram_used_gb)} / ${root.gb(root.gpuAt(1)?.vram_total_gb)}` },
                    { label: qsTr("Potencia"), value: root.watt(root.gpuAt(1)?.power_w) }
                ]
                : [
                    { label: qsTr("Descarga"), value: root.speed(root.network?.rx_mbps) },
                    { label: qsTr("Subida"), value: root.speed(root.network?.tx_mbps) },
                    { label: qsTr("Interfaz"), value: root.network?.interface ?? "—" }
                ]
        }

        MetricCard {
            width: (cards.width - cards.columnSpacing) / 2
            title: root.battery?.present ? qsTr("Batería") : qsTr("Sistema")
            ...
            subtitle: root.battery?.present ? (root.battery?.status ?? "—") : qsTr("Estado actual del equipo")
            progress: root.battery?.present ? Number(root.battery?.percent ?? 0) / 100 : -1
            rows: root.battery?.present
                ? [
                    { label: qsTr("Potencia"), value: root.watt(root.battery?.power_w) },
                    { label: qsTr("Red"), value: root.network?.interface ?? "—" },
                    { label: qsTr("Descarga / subida"), value: `${root.speed(root.network?.rx_mbps)} / ${root.speed(root.network?.tx_mbps)}` }
                ]
                : [
                    { label: qsTr("Carga de 1 min"), value: root.snapshot?.load?.length ? root.number(root.snapshot.load[0], 2) : "—" },
                    { label: qsTr("Refrigeración"), value: root.fans.length > 0 ? `${root.fans.length} ventiladores` : qsTr("Pasiva") },
                    { label: qsTr("Red"), value: root.network?.interface ?? "—" }
                ]
        }
    }
    ...
    Row {
        ...
        SummaryStat {
            ...
            label: qsTr("Refrigeración")
            value: root.fans.length > 0
                ? root.fans.map(f => `${f.rpm} RPM`).join(" · ")
                : qsTr("Sin datos de ventiladores")
        }

        SummaryStat {
            width: (parent.width - parent.spacing * 3) / 4
            label: qsTr("Carga del sistema")
        ...
        SummaryStat {
            width: (parent.width - parent.spacing * 3) / 4
            label: qsTr("Threads")
            value: `${root.cpu?.cores ?? 0} ${qsTr("lógicos")}`
        }
    }
