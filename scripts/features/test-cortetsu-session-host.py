...
for marker in ('name: "session"', "WlrLayer.Overlay", "WlrKeyboardFocus.Exclusive", "CortetsuShellState.forActive()", "Session.Content"):
    assert marker in host, marker
assert "SessionHost {}" in shell
assert "id: session" in panels and "visible: false" in panels
print("PASS: session is a first-party focused surface and the legacy panel is hidden")
