...
ICONS = (ROOT / "cortetsu/utils/Icons.qml").read_text()
...
def main() -> None:
    ...
    require(HUB, "window.initialClass, window.class", "clase inicial y actual de la ventana")
    require(HUB, "steam://rungameid/${steamAppId}", "asociación de juegos Steam por app id")
    require(HUB, 'const identity = String(window.initialClass ?? "").trim()', "identidad estable de la ventana")
    require(HUB, '`window:${window.address ?? anonymousWindowIndex++}`', "clave única de ventana anónima")
    require(HUB, "const executableMatches = entries.filter(candidate =>", "resolución por ejecutable desktop")
    if not (
        HUB.index("DesktopEntries.byId(identity)")
        < HUB.index("const matches = entries.filter(candidate =>")
        < HUB.index("DesktopEntries.heuristicLookup(identity)")
        < HUB.index("const executableMatches = entries.filter(candidate =>")
    ):
        raise SystemExit("FAIL: el resolver debe probar IDs, clases, heurísticas y ejecutables en ese orden")
    require(HUB, "if (matches.length === 1)", "coincidencias normalizadas no ambiguas")
    forbid(HUB, 'replace(/[^a-z0-9]/g', "normalización de identidad con colisiones por puntuación")
    require(VIEW, "visible: root.trayVisible && root.trayItems.length > 0", "tray oculto cuando no contiene iconos")
    require(ICONS, 'if (value.startsWith("file://") || value.startsWith("/"))', "iconos desktop con ruta absoluta")
    require(ICONS, "value.match(/^(?:steam_app|steam_icon)_([0-9]+)$/i)", "icono cache de Steam desde la entrada desktop")
    ...
    require(CANONICAL_HYPR, 'create_bind(vars.kbShowSidebar, hl.dsp.global("cortetsu:sidebar"))', "SUPER+N a notificaciones")
    ...
    forbid(HUB, "state.launcher = !wasOpen;\n        shown = true;", "launcher resucitando BottomHub")
    ...
    require(SHORTCUTS, "const open = !state.sidebar;", "sidebar solo abre notificaciones")
    require(SHORTCUTS, "state.utilities = false;", "ajustes rápidos separados de notificaciones")
    require(HUB, "const wasOpen = state.sidebar;", "botón de notificaciones independiente")
    ...
    require(PANELS, "anchors.bottomMargin: CortetsuDesign.bottomHubHeight + CortetsuDesign.spacingStandard", "OSD sobre BottomHub")
