...
RESOLVER = (MODULES / "BottomHubResolver.js").read_text()
VIEW = (MODULES / "CortetsuBottomHubView.qml").read_text()
TRAY = (MODULES / "CortetsuTraySegment.qml").read_text()
TRAY_MENU = (MODULES / "bar/popouts/CortetsuTrayMenu.qml").read_text()
...
def main() -> None:
    ...
    if not (
        ...
        < RESOLVER.index("const matches = entries.filter(candidate =>")
        < RESOLVER.index("const steamAppId = identities")
        < RESOLVER.index("const executableMatches = entries.filter(candidate =>")
    ):
        raise SystemExit("FAIL: el resolver debe probar IDs, clases, Steam y ejecutables en ese orden")
    require(RESOLVER, "if (matches.length === 1)", "coincidencias normalizadas no ambiguas")
    forbid(RESOLVER, "heuristicLookup", "búsqueda aproximada como identidad de ventana")
    forbid(RESOLVER, 'replace(/[^a-z0-9]/g', "normalización de identidad con colisiones por puntuación")
